...
<tr class="relative z-0 my-10" style="opacity: 1; transform-origin: 50% 50% 0px;">
  <td class="rounded-bl-full rounded-tl-full bg-navy-750">
    <div
      ...
      class="mr-1 flex h-[96px] w-[96px] items-center justify-center rounded-full border transition {data.mode ===
      'underdog'
        ? 'border-purple-500 bg-[#110515] shadow-[inset_0_0_0.8rem_0.4rem_#4D2858]'
        : 'border-[#30663F] bg-[#001A07] text-2xl shadow-[inset_0_0_0.8rem_0.4rem_#1A3024]'}"
    >
      <div
        class="flex h-[56%] w-[56%] items-center justify-center rounded-full border-2 text-center font-bold transition {data.mode ===
        'underdog'
        ...
        {#if !data.finished}
          <a
            class="button {data.mode === 'underdog'
              ? 'button-pink-dimmed'
              : 'button-green-dimmed'} ml-1 mr-5 w-auto flex-1 px-1 transition-all hover:brightness-110"
